...
side effect runs once per pod unless something elects an owner. This module
holds the two election shapes the proxy needs and the role gate that lets an
operator move the work off the pods serving traffic.
"""
...
class JobRole(Enum):
    ...
    def runs_single_owner_jobs(self) -> bool:
        """False only for a pod dedicated to serving traffic."""
...
class JobLease(Enum):
    ...
    Only LEADER and UNGUARDED ever reach a job body: a FOLLOWER has nothing to do,
    so ``run_as_single_owner`` returns without calling it.
    """
...
async def run_as_single_owner(
    *,
    ...
    Returns ``None`` without running when another pod owns this tick. The lease is
    handed to ``run`` so a job that can degrade under an unguarded run (skipping a
    destructive prune, say) can see which case it is in.
...
async def claim_once_per_window(
    *,
    ...
    # A deployment with no Redis at all gets ``None`` and runs, since it has no
    # peer to duplicate against. A configured-but-unreachable Redis gets ``False``
    # and skips, because these windows end at someone's inbox
...
async def _renew_until_cancelled(*, manager: PodLockManager, job_name: str, ttl_seconds: int) -> None:
    ...
    while True:
        ...
        if not await manager.renew_lock(cronjob_id=job_name, ttl=ttl_seconds):
            # Another pod owns the lease now. Stop renewing so it keeps it; the
            # release on the way out compares owners and will not steal it back
            verbose_proxy_logger.warning(
