...
export const getRequiredModels = (
  config: Pick<ComplexityRouterConfigPayload, "tiers" | "classifier_llm_config" | "embedding_model"> & {
    /** A pinned default model. Not part of the payload - it rides on litellm_params - but it is a
     * model reference the deployment carries all the same, so it has to resolve like the rest. */
...
export const getReferencedModelsError = (
  params: {
    ...
    /** A pinned default model, if the caller set one. Always submitted, so always checked. */
    defaultModel?: string;
  },
  availability: ModelAvailability,
): string | null => {
