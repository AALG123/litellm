...
export interface ComplexityRouterConfigValue {
  ...
  /**
   * An explicit default model, written to litellm_params.complexity_router_default_model on submit.
   * Unset means the default tracks the tiers - see resolveComplexityDefaultModel. Deliberately not
   * part of ComplexityRouterConfigPayload: a default_model inside complexity_router_config is
   * overwritten by the deployment-level one (router.py: init_complexity_router_deployment), so
   * litellm_params stays the single source of truth for it.
   */
...
const ComplexityRouterConfig: React.FC<ComplexityRouterConfigProps> = ({
  ...
  // What submit will write as complexity_router_default_model, so the fallback option is offered
  // exactly when a default will exist and the row below can show what the tiers currently imply.
      ...
      <Card>
        ...
        <div className="mb-2">
          ...
          <AntdSelect
            ...
            placeholder={
              derivedDefaultModel
                ...
                : "Add a model to a tier, or pick one here"
            }
          ...
          <Text type="secondary" style={{ display: "block", marginTop: 4, fontSize: 12 }}>
            Used when the tier the request lands in has no model, and when the classifier fails with
            &quot;Route to the default model&quot; selected.
          </Text>
        </div>
      </Card>
