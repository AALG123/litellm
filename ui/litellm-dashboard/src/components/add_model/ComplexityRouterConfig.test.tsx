...
describe("ComplexityRouterConfig default model", () => {
  ...
  it("shows what the tiers currently imply, so an untouched router still names its default", () => {
    renderWithProviders(<ComplexityRouterConfig {...baseProps} />);
    // MEDIUM first, matching router.py's derivation order.
  ...
  it("asks for a model rather than naming a derived one when no tier holds one", () => {
    ...
    expect(screen.getByText("Add a model to a tier, or pick one here")).toBeInTheDocument();
  });
  ...
  it("shows a pinned model as the selection instead of the tier-derived one", () => {
    ...
    expect(within(getDefaultModelSelect().closest(".ant-select") as HTMLElement).getByTitle("claude-3-opus")).toBeInTheDocument();
  });

  it("unlocks the default model fallback on a pin alone, with no tier to derive from", () => {
    // The pin is submitted as complexity_router_default_model just like a derived value, so the
    // fallback destination exists even though the tiers are empty.
