...
const authorizedRoleMock = vi.fn(() => ({ isViewOnly: false }));
...
describe("ShadowEvalSection", () => {
  beforeEach(() => {
    authorizedRoleMock.mockReturnValue({ isViewOnly: false });
  });
  ...
  it("hides the start form and stop button from view-only admins", () => {
    authorizedRoleMock.mockReturnValue({ isViewOnly: true });
  ...
  it("surfaces the last failure so a growing failed_count is diagnosable", () => {
  ...
  it("renders nothing for non-admins when the proxy answers 403", () => {
    ...
    const { container } = render(<ShadowEvalSection accessToken="token" />);
