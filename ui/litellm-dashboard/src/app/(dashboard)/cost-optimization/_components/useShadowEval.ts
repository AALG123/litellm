...
export const useStartShadowEval = () => {
  ...
  return useMutation({
    mutationFn: async (body: StartShadowEvalRequest) => {
      const { data } = await fetchClient.POST("/auto_router/shadow_eval/start", { body });
      return data;
    },
    onSuccess: () => invalidateShadowEval(queryClient),
    onError: (error: unknown) =>
      NotificationsManager.fromBackend(error instanceof Error ? error.message : "Failed to start shadow eval"),
  });
};
...
export const useStopShadowEval = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => invalidateShadowEval(queryClient),
    onError: (error: unknown) =>
      NotificationsManager.fromBackend(error instanceof Error ? error.message : "Failed to stop shadow eval"),
  });
};
